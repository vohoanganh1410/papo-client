	...
	{},
	{
		[ INVITES_REQUEST_SUCCESS ]: ( state, action ) => {
			...
			action.invites.forEach( invite => {
				...
				const user = pick( invite.user, 'login', 'email', 'name', 'avatar_URL', 'facebookId', 'displayName' );
				const invitedBy = pick( invite.invited_by, 'name', 'login', 'avatar_URL', 'facebookId', 'displayName'  );
