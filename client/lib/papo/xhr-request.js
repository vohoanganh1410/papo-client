...
import PAPOError from './papo-error';
import config from 'config';
import * as OAuthToken from 'lib/oauth-token';
/**
 * Module variables
 */
const debug = debugFactory( 'papo-xhr-request' );
		...
		if ( ok ) {
			if ( isEnvelopeMode && processResponseInEnvelopeMode ) {
				...
				if ( null !== statusCode && 2 !== Math.floor( statusCode / 100 ) ) {
					...
					const wpe = PAPOError( { path, method }, statusCode, body );
...
export default function request( options, fn ) {
	...
	if ( isRestAPI ) {
		...
		basePath = '/' + apiNamespace;	// e.g: /oauth
	} else {
		basePath = '/papo-json'; // /papo-json/sites/%s/sd/v2 (deprecated)
	}
