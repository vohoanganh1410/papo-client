 ...
 * debug instance
 */
const debug = debugFactory( 'proxy-request' );

/**
 * WordPress.com REST API base endpoint.
 */
const proxyOrigin = config ( 'api_url' );
...
function submitRequest( params ) {
	...
	// console.log(iframe.contentWindow.postMessage( 'sdfsf', proxyOrigin ));
...
function install() {
	...
	iframe.src = proxyOrigin + '/rest-proxy/?v=2.0#' + origin;
...
function onmessage( e ) {
	...
	if ( statusCode && 2 === Math.floor( statusCode / 100 ) ) {
		...
		const wpe = PAPOError( params, statusCode, body );
