...
import { createReadStream } from 'lib/papo/papo-org/utils/fs';
 ...
 */
function buildFormData( files, page_id ) {
	const formData = [];
	...
	for ( i = 0; i < files.length; i++ ) {
		...
		formData.push( [ 'files', f ] );
		formData.push( [ 'page_id', page_id ] );
	}
 ...
 * @return {Null} null
 */
export default function Image( id, sid, wpcom ) {
	if ( ! ( this instanceof Image ) ) {
		return new Image( id, sid, wpcom );
	}
 ...
 */
Image.prototype.get = function( query = {}, fn ) {
	query.apiVersion = query.apiVersion || '1.2';
	const path = '/pages/' + this._sid + '/media/' + this._id;
 ...
 * @return {Function} request handler
 */
Image.prototype.update = function( query, body, fn ) {
	const params = { path: '/pages/' + this._sid + '/media/' + this._id };
 ...
 * @return {Function} request handler
 */
Image.prototype.edit = function( query, body, fn ) {
	...
	const params = { path: '/pages/' + this._sid + '/media/' + this._id + '/edit' };
 ...
 * @return {Function} request handler
 */
Image.prototype.addFiles = function( query, files, fn ) {
	if ( undefined === fn ) {
		if ( undefined === files ) {
			files = query;
			query = {};
	...
	const params = {
		path: '/files',
		formData: buildFormData( files, this._sid ),
	};
	// alert("sdf")
	// query.page_id = this._sid;
 ...
 * @return {Function} request handler
 */
Image.prototype.addUrls = function( query, media, fn ) {
	if ( undefined === fn ) {
		if ( undefined === media ) {
			media = query;
			query = {};
	...
	const path = '/pages/' + this._sid + '/media/new';
 ...
 * @return {Function} request handler
 */
Image.prototype.delete = Image.prototype.del = function( query, fn ) {
	const path = '/pages/' + this._sid + '/media/' + this._id + '/delete';
