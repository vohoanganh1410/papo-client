...
import ConversationQueryKey from './key';
import { DEFAULT_CONVERSATION_QUERY } from './constants';

/**
 * PostQueryManager manages posts which can be queried and change over time
 */
export default class ConversationQueryManager extends PaginatedQueryManager {
	static QueryKey = ConversationQueryKey;
	static DefaultQuery = DEFAULT_CONVERSATION_QUERY;
	...
	static compare( query, postA, postB ) {
		...
		switch ( query.order_by ) {
			...
			case 'date':
			default:
				order = new Date( postA.created_time ) - new Date( postB.created_time );
		}
