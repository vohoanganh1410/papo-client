...
fieldMasks[ 'customer-name' ] = {
	mask: function( previousValue, nextValue ) {
		...
		return capitalLetter(nextValue);
	},

	unmask: identity,
};

fieldMasks[ 'order-note' ] = {
	mask: function( previousValue, nextValue ) {
		// If the user is deleting from the value then don't modify it
		// if ( previousValue && previousValue.length > nextValue.length ) {
		// 	return nextValue;
...
fieldMasks.number = {
	mask: function( previousValue, nextValue ) {
		return nextValue; //formatCreditCard( nextValue );
	},
...
fieldMasks.mobile = {
	mask: function( previousValue, nextValue ) {
		return nextValue.replace( /[^\d]/g, '' ).substring( 0, 11 );
	},
