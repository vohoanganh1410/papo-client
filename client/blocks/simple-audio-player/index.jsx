...
export default class ReactRoundPlayer extends Component {
	constructor( props ) {
		...
		this.colorScale = this.props.colorScale
			...
			: [ '#980000', '#fa0000', '#ffc7c7', '#ffe1e1', '#ffffff' ];
	}
			...
			>
				<g>
					...
					>
						<tspan
							fill={ this.colorScale[ 0 ] }
							style={ { fontSize: radius / 2, lineHeight: radius / 2, fontFamily: 'Verdana' } }
							x="0"
							textAnchor="middle"
						>
		...
		return (
			<div className={ classNames( g_styles.d_flex, g_styles.v_center ) }>
				...
				>
					...
					<g>{ this.getCircle( radius, radius, radius * centerWidth, this.colorScale[ 4 ] ) }</g>
					<g fillOpacity="0.5">
						{ this.getProgressArc(
							radius,
							radius,
				...
				/>
				<span className={ g_styles.pl_10 }>{ this.props.name }</span>
			</div>
		);
	}
}
