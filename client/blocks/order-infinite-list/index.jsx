...
export default class OrderList extends React.PureComponent {
	...
	isOrder( t ) {
		var a = this.rows[ t.index ];
		return a && ( 'order' === a.type || 'foreword' === a.type );
	}
	...
	renderDateGroup = date => {
		return <DateSeparator className={ styles.date_separator } key={ date } date={ new Date( date ) } />;
	};
	...
	renderRow = ( key, a ) => {
		...
		switch ( row.type ) {
			...
			case 'order':
				return this.renderOrder( row.data, {
					hasFocus: r,
					isHovered: i,
					page_id: row.page_id,
				} );
	...
	render() {
		...
		const rows = this.makeSeedData( 20 );
