...
const patchDocumentClass = toClass => ( next = null, prev = null ) => {
	if ( prev === next ) {
		return;
	}

	if ( prev ) {
		document.documentElement.classList.remove( toClass( prev ) );
	}

	if ( next ) {
		document.documentElement.classList.add( toClass( next ) );
	}
};
...
const patchHtmlClass = patchDocumentClass( d => d );
...
export default class BodySectionCssClass extends React.Component {
	componentDidMount() {
		...
		patchHtmlClass( this.props.documentClass );
	}

	componentDidUpdate( prevProps ) {
		patchHtmlClass( this.props.documentClass, prevProps.documentClass );
