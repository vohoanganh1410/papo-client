...
class FoldableCard extends Component {
	...
	renderActionButton() {
		...
		if ( this.props.children ) {
			...
			const expandIcon = classNames( 'ts_icon', {
				ts_icon_caret_down: ! this.state.expanded,
				ts_icon_caret_up: this.state.expanded,
			} );
			return (
				<button
	...
	renderHeader() {
		...
		const headerIconClasses = classNames( 'ts_icon', this.props.icon, {
			small_right_margin: true,
		} );
