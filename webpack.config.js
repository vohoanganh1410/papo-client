...
const isCalypsoClient = process.env.PAPO_CLIENT === 'true';
...
function getWebpackConfig( { cssFilename, externalizeWordPressPackages = false } = {} ) {
	cssFilename =
		cssFilename || ( isDevelopment || calypsoEnv === 'desktop' ? '[name].css' : '[chunkhash].css' );
	...
	const webpackConfig = {
		...
		output: {
			...
			publicPath: '/papo/',
			filename: '[chunkhash].min.js', // prefer the chunkhash, which depends on the chunk, not the entire build
			chunkFilename: '[chunkhash].min.js', // ditto
		...
		module: {
			...
			rules: [
				...
				},
				{
					...
					test: /\.json$/,
					include: [ path.resolve( __dirname, 'client/i18n' ) ],
					exclude: [ /en\.json$/ ],
					use: [
						{
							loader: 'file-loader?name=client/i18n/[name].[hash].[ext]',
						},
					],
				},
				{
